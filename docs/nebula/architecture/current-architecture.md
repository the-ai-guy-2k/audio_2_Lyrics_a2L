...
No verse/chorus labels are invented. Vocal isolation is not in the pipeline. Song title and artist are operator-supplied and optional; the WAV filename is not used as a title. After approval the operator Output screen shows a Standard Lyric Sheet derived from the approved artifact; canonical TXT/JSON remain authoritative. Downloaded files are named from that metadata when present. The Album screen organizes existing Song Release Records. Release Readiness reports A2L INTERNAL READY/INCOMPLETE from current governed records; it is not distributor or commercial readiness. Album **Export release package** writes a derived ZIP of that current truth; it does not copy master audio or invent missing release data. The locked Jay song's **faster-whisper** approved lyrics (ACI-A2L-009 Amendment 01, revision 2) were not modified. Parakeet output is a separate unapproved chain. Rhythm + structure Song Intelligence remains a candidate spike (ACI-A2L-SI-002 / ACI-A2L-SI-003) and is not in this product path. HTDemucs was used only as diagnostic analyzer plumbing in SI-003 and is not an A2L source-separation capability. Key + Mode remains a candidate spike (ACI-A2L-SI-004; librosa chroma_cqt + Krumhansl–Schmuckler) and is not in this product path. Audio Intelligence remains a candidate spike and is not in this product path. CLAP (ACI-A2L-SI-007 / ACI-A2L-SI-008; `laion/larger_clap_music` on CPU) remains the Energy / Intensity and acoustic/electronic path; unique-label instrumentation and genre were not resolved by CLAP. SI-009 evaluated one specialized AudioSet AST tagger (`MIT/ast-finetuned-audioset-10-10-0.4593`) for those two categories: genre/style earned an engineering WIN; instrumentation remains NO WIN. This is not in the product path. Lyric Intelligence remains a candidate spike (ACI-A2L-SI-006; deterministic NLP on approved lyrics) and is not in this product path.
